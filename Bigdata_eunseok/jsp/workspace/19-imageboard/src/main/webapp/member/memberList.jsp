...
<body>
	<table border="1" width="690">
		...
		<tr>
			...
			<%if(startPage > 3){ %>
				[<a class="paging" href="../main/index.jsp?req=memberList&pg=<%=startPage-1%>">이전</a>]
			<%} %>
			<%for(int i = startPage; i <= endPage; i++) {%>
				<%if(pg==i){ %>
					[<a class="currentPaging" href="../main/index.jsp?req=memberList&pg=<%=i%>"><%=i %></a>]
				<%} else {%>
					[<a class="paging" href="../main/index.jsp?req=memberList&pg=<%=i%>"><%=i %></a>]
				<%} %>
			<%} %>
			<%if(endPage < total_page){ %>
				[<a class="paging" href="../main/index.jsp?req=memberList&pg=<%=endPage+1%>">다음</a>]
			<%} %>
			</td>
		</tr>
	</table>
