...
<body>
		...
		<c:forEach var="bean" items="${list }">
			<tr>
				...
				<td width="200">
					...
					<a href="index.jsp?req=boardDetail&pg=${pg }&board_num=${bean.board_num }">
						${bean.board_subject }
					</a>
				</td>
		...
		<tr>
			<td colspan="5" align="center">
				<c:if test="${startPage > 3 }">
					[<a class="paging" 
							href="index.jsp?req=boardList&pg=${startPage - 1 }">
						이전</a>]
				</c:if>
				<c:forEach var="i" begin="${startPage }" end="${endPage }" step="1">
					<c:if test="${pg == i }">
						[<a class="currentPaging" 
							href="index.jsp?req=boardList&pg=${i }">${i }</a>]
					</c:if>
					<c:if test="${pg != i }">
						[<a class="paging" 
							href="index.jsp?req=boardList&pg=${i }">${i }</a>]
				...
				<c:if test="${endPage < total_page }">
					[<a class="paging" 
							href="index.jsp?req=boardList&pg=${endPage }">
						다음</a>]
				</c:if>
			</td>
		</tr>
	...
	<a href="index.jsp?req=boardWriteForm">글쓰기</a>
</body>
</html>