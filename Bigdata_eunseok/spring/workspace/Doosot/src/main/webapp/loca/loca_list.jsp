...
<link rel="stylesheet" type="text/css" href="../css/main_page.css?v=2">
<link rel="stylesheet" type="text/css" href="../css/loca.css?v=3">
		...
		<div class="content">
			...
			<!-- 페이징 -->
				<tr height="70">
					...
					<c:if test="${startPage > 3}">
							<a class="paging" href="loca_list.do?pg=${startPage-1}">◀&nbsp;</a>
					</c:if>
					
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
							<c:if test="${pg==i}">
								<a class="currentPaging" href="loca_list.do?pg=${i}">${i}</a>
							</c:if>
							<c:if test="${pg!=i}">
								<a class="paging" href="loca_list.do?pg=${i}">${i}</a>
							</c:if>
					</c:forEach>
									
					<c:if test="${endPage < totalP}">
							<a href="loca_list.do?pg=${endPage+1}" class="paging">&nbsp;▶</a>
					</c:if>				
					</td>
				</tr>
			</table>
			...
			<div>
				<c:if test="${admin_num == '1' }">
					...
					<input type="button" value="점포 삭제" class="del_btn" onclick="location.href='../loca/loca_deleteForm.jsp?pg=${pg}'">
				</c:if>
			</div>
